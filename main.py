...
def is_open_palm(lm): return all(is_finger_up(lm,t,p) for t,p in [(HL.INDEX_FINGER_TIP.value,HL.INDEX_FINGER_PIP.value),(HL.MIDDLE_FINGER_TIP.value,HL.MIDDLE_FINGER_PIP.value),(HL.RING_FINGER_TIP.value,HL.RING_FINGER_PIP.value),(HL.PINKY_TIP.value,HL.PINKY_PIP.value)])
def is_closed_fist(lm): return not any(is_finger_up(lm,t,p) for t,p in [(HL.INDEX_FINGER_TIP.value,HL.INDEX_FINGER_PIP.value),(HL.MIDDLE_FINGER_TIP.value,HL.MIDDLE_FINGER_PIP.value),(HL.RING_FINGER_TIP.value,HL.RING_FINGER_PIP.value),(HL.PINKY_TIP.value,HL.PINKY_PIP.value)])
def is_peace_sign(lm): return is_finger_up(lm,HL.INDEX_FINGER_TIP.value,HL.INDEX_FINGER_PIP.value) and is_finger_up(lm,HL.MIDDLE_FINGER_TIP.value,HL.MIDDLE_FINGER_PIP.value) and not is_finger_up(lm,HL.RING_FINGER_TIP.value,HL.RING_FINGER_PIP.value) and not is_finger_up(lm,HL.PINKY_TIP.value,HL.PINKY_PIP.value)
def is_pointing(lm): return is_finger_up(lm,HL.INDEX_FINGER_TIP.value,HL.INDEX_FINGER_PIP.value) and not is_finger_up(lm,HL.MIDDLE_FINGER_TIP.value,HL.MIDDLE_FINGER_PIP.value) and not is_finger_up(lm,HL.RING_FINGER_TIP.value,HL.RING_FINGER_PIP.value) and not is_finger_up(lm,HL.PINKY_TIP.value,HL.PINKY_PIP.value)
def is_thumb_up(lm): return lm[HL.THUMB_TIP.value].y<lm[HL.THUMB_IP.value].y and all(not is_finger_up(lm,t,p) for t,p in [(HL.INDEX_FINGER_TIP.value,HL.INDEX_FINGER_PIP.value),(HL.MIDDLE_FINGER_TIP.value,HL.MIDDLE_FINGER_PIP.value),(HL.RING_FINGER_TIP.value,HL.RING_FINGER_PIP.value),(HL.PINKY_TIP.value,HL.PINKY_PIP.value)])
def is_thumb_down(lm): return lm[HL.THUMB_TIP.value].y>lm[HL.THUMB_IP.value].y and all(not is_finger_up(lm,t,p) for t,p in [(HL.INDEX_FINGER_TIP.value,HL.INDEX_FINGER_PIP.value),(HL.MIDDLE_FINGER_TIP.value,HL.MIDDLE_FINGER_PIP.value),(HL.RING_FINGER_TIP.value,HL.RING_FINGER_PIP.value),(HL.PINKY_TIP.value,HL.PINKY_PIP.value)])
def is_ok_sign(lm): return math.hypot(lm[HL.INDEX_FINGER_TIP.value].x-lm[HL.THUMB_TIP.value].x, lm[HL.INDEX_FINGER_TIP.value].y-lm[HL.THUMB_TIP.value].y)<0.05 and is_finger_up(lm,HL.MIDDLE_FINGER_TIP.value,HL.MIDDLE_FINGER_PIP.value)
def is_pinch(lm): return math.hypot(lm[HL.INDEX_FINGER_TIP.value].x-lm[HL.THUMB_TIP.value].x, lm[HL.INDEX_FINGER_TIP.value].y-lm[HL.THUMB_TIP.value].y)<0.03
...
def select_puzzle():
    ...
    while True:
        ...
        cv2.putText(frame, "Use W/S keys, ENTER to select, Q=quit", (50, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
        
        cv2.imshow("Select Puzzle", frame)
        key = cv2.waitKey(1) & 0xFF
        
        if key == ord('w'):           # Move selection up
            selection = (selection - 1) % len(puzzles)
...
# ---------- Select Puzzle Before Main Loop ----------
chosen_puzzle = select_puzzle()
if chosen_puzzle is None:
    print("No puzzle selected. Exiting.")
    exit()
...
chosen_puzzle = select_puzzle()
if chosen_puzzle is None:
    print("No puzzle selected. Exiting."); exit()
MODEL_FILES = PUZZLE_MODELS[chosen_puzzle]

# ---------- Pieces Data Setup ----------
pieces_data = []

# Define starts and corrects per puzzle
...
if missing:
    print("ERROR: Missing model files:"); print("\n".join(missing))
    raise SystemExit("Add missing model files and re-run.")
...
for p in pieces_data:
    ...
    p["pos"]=p["start"].copy(); p["grabbed"]=False; p["snapped"]=False

move_smooth=0.25; grabbed_obj_id=None; PICK_THRESHOLD=1.5
score=0; total_pieces=len(pieces_data)
...
# ---------- Main Loop ----------
last_reset=0; reset_cooldown=1; running=True
...
while running:
    ...
    if results.multi_hand_landmarks:
        ...
        if open_palm and grabbed_obj_id is None:
            pick_id=select_nearest_piece(hand_world)
        ...
        feedbacks = [(point,"👉 Pointing detected"),(ok_sign,"👌 OK detected"),(thumb_up,"👍 Thumbs Up"),
                     (thumb_down,"👎 Thumbs Down"),(pinch,"🤏 Pinch detected")]
    ...
    for p in pieces_data:
        if p["snapped"]: continue
        dx,dy,dz=p["pos"][0]-p["correct"][0],p["pos"][1]-p["correct"][1],p["pos"][2]-p["correct"][2]
        if math.sqrt(dx*dx+dy*dy+dz*dz)<0.35: p["pos"]=p["correct"].copy(); p["grabbed"]=False; p["snapped"]=True
    ...
    if score==total_pieces: cv2.putText(frame,"🎉 You Win! Puzzle Completed! 🎉",(w//4,h//2),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),3)
